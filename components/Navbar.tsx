      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-5 h-14 flex items-center justify-between gap-3">
          ...
          <Link
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-semibold transition-colors ${
              isActive('/videos')
                ...
                : 'bg-white/5 text-[#9b98b0] hover:text-[#e8e6f0] hover:bg-white/10'
            }`}
          >
            <span>🎬</span>
            <span>Videos</span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className="relative md:hidden" ref={mobileMenu.ref}>
            ...
            {mobileMenu.open && (
              <div className="absolute top-full right-0 mt-1.5 w-64 bg-[#1a1830] rounded-xl border border-white/8 shadow-xl shadow-black/40 overflow-hidden z-50">
                <div className="py-1">
                  <Link
                    href="/profile"
                    onClick={() => mobileMenu.setOpen(false)}
                    className={`flex items-center gap-3 px-4 py-3 text-sm transition-colors hover:bg-white/5 ${isActive('/profile') ? 'text-[#9b8cf5]' : 'text-[#e8e6f0]'}`}
                  >
                    <svg className="w-4 h-4 text-[#9b98b0]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                    ...
                    {userLevel !== null && (
                      <span className="ml-auto text-xs bg-[#7c6df2]/20 text-[#9b8cf5] px-1.5 py-0.5 rounded font-bold">Lv {userLevel}</span>
                    )}
                  </Link>
                  <Link
                    href="/forecast"
                    onClick={() => mobileMenu.setOpen(false)}
                    className={`flex items-center gap-3 px-4 py-3 text-sm transition-colors hover:bg-white/5 ${isActive('/forecast') ? 'text-[#9b8cf5]' : 'text-[#e8e6f0]'}`}
                  >
                    <svg className="w-4 h-4 text-[#9b98b0]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                    </svg>
